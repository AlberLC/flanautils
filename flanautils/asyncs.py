...
def _process_function(func: Callable, *args: Any, queue__: multiprocessing.Queue, **kwargs: Any) -> None:
...
def do_every(
    ...
    **kwargs: Any
    ...
    async def do_every_(times_: int) -> list | Never:
        if times_ is None:
            if do_first_now:
...
async def poll_process(process: multiprocessing.Process, sleep_seconds=1) -> None:
    """
...
async def run_process(func: Callable, *args: Any, timeout: int | float | None = None, **kwargs: Any) -> Any:
    """
    ...
    await wait_for_process(
        multiprocessing.Process(target=_process_function, args=(func, *args), kwargs={'queue__': queue__, **kwargs}),
        timeout
    )
    try:
...
async def wait_for_process(process: multiprocessing.Process, timeout: int | float | None = None) -> None:
    """
